...
void Laborator5::Init()
{
	...
	beforeprojection = projectionMatrix;
}
...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	//ale mele
	{
		...
		modelMatrix = glm::rotate(modelMatrix, RADIANS(240.0f), glm::vec3(1, 0, 0));
		RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
	}
	{
		glm::mat4 modelMatrix = glm::mat4(1);
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_O)) {
				...
				glm::mat4 Projection = glm::ortho(0.0f, (float)resolution.x, (float)resolution.y, 0.0f, 0.0f, 1.0f);
		...
		if (window->KeyHold(GLFW_KEY_B)) {
			...
			projectionMatrix = beforeprojection;
		}
		...
		if(window->KeyHold(GLFW_KEY_N)) {
			...
			resolution.x -= deltaTime;
			resolution.y -= deltaTime;
			projectionMatrix = beforeprojection;
			glm::mat4 Projection = glm::ortho(0.0f, (float)resolution.x, (float)resolution.y, 0.0f, 0.0f, 1.0f);
			projectionMatrix *= Projection;
		}
...
void Laborator5::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->GetSpecialKeyState() == 0) {
			...
			if(mouseX>0)
				camera->RotateFirstPerson_OX(sensivityOX);
			else
				camera->RotateFirstPerson_OX(-sensivityOX);
			
			if(mouseY>0)
				camera->RotateFirstPerson_OY(sensivityOY);
			else
				camera->RotateFirstPerson_OY(-sensivityOY);
		...
		if (window->GetSpecialKeyState() && GLFW_MOD_CONTROL) {
			...
			if (mouseX > 0)
				camera->RotateThirdPerson_OX(sensivityOX);
			else
				camera->RotateThirdPerson_OX(-sensivityOX);
			
			if (mouseY > 0)
				camera->RotateThirdPerson_OY(sensivityOY);
			else
				camera->RotateThirdPerson_OY(-sensivityOY);
