...
void Laborator8::Init()
{
	...
	}
	{
		...
		shader->AddShader("Source/Laboratoare/Laborator8/Shaders/Vertex.glsl", GL_VERTEX_SHADER);
		shader->AddShader("Source/Laboratoare/Laborator8/Shaders/Fragment.glsl", GL_FRAGMENT_SHADER);
		shader->CreateAndLink();
		shaders[shader->GetName()] = shader;
	}
...
void Laborator8::Update(float deltaTimeSeconds)
{
		...
		//s = "shader";
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders[s], modelMatrix);
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders[s], modelMatrix);
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders[s], modelMatrix, glm::vec3(0, 0.5, 0));
	}
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders[s], modelMatrix);
	}
